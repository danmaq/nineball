...
namespace danmaq.Nineball.core.raw {
	...
	public sealed class CAudio : IDisposable {
		...
		/// </summary>
		private readonly CallbackIndex2Asset INDEX2ASERT;
		...
		public float volumeBGM {
			set {
				if( engine != null ) { engine.GetCategory( "Music" ).SetVolume( value ); }
			}
		}
		...
		public float volumeSE {
			set {
				if( engine != null ) { engine.GetCategory( "Default" ).SetVolume( value ); }
			}
		}
		...
		public void Dispose() {
			...
			string strErr = "";
			try { if( cueBGM != null ) { cueBGM.Dispose(); } }
			catch( Exception e ) { strErr += e.ToString() + "\r\n"; }
			try { if( soundBank != null ) { soundBank.Dispose(); } }
			catch( Exception e ) { strErr += e.ToString() + "\r\n"; }
			try { if( waveBankBGM != null ) { waveBankBGM.Dispose(); } }
			catch( Exception e ) { strErr += e.ToString() + "\r\n"; }
			try { if( waveBankSE != null ) { waveBankSE.Dispose(); } }
			catch( Exception e ) { strErr += e.ToString() + "\r\n"; }
		...
		private IEnumerator<object> threadPlaySE() {
			while( !( engine == null || soundBank == null ) ) {
				for( int i = 0; i < ( isHighPriorityMode ? 2 : 5 ); i++ ) { yield return null; }
				...
				foreach( ushort index in RESERVED_SE ) {
					...
					if( bExist ) {
						if( cueSE.time < nCount - 10 ) {
							if( cueSE.cue.IsPlaying ) { cueSE.cue.Pause(); }
		...
		private IEnumerator<object> threadGC() {
			while( true ) {
				for( int i = 0; i < ( isHighPriorityMode ? 6 : 20 ); i++ ) { yield return null; }
				if( GRAVE_SE.Count > 0 ) {
