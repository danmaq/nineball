package danmaq.nineball.core{
	...
	public final class CTaskManager{
		 ...
		 * @return 登録されているタスクの数
		 */
		public function get num():uint{ return m_list.length; }
		...
		public function add( task:ITask, bSkipInitialize:Boolean = false ):void{
			...
			if( !bInserted ){ m_list.push( task ); }
			if( !bSkipInitialize ){ task.initialize(); }
		}
		...
		public function eraseAll():uint{
			...
			for each( var task:ITask in m_list ){ task.dispose(); }
		 ...
		 */
		public function toString():String{ return StringUtil.substitute( "TaskNum:{0}", num ); }
	}
}
