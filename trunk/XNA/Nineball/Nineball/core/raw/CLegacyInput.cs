...
namespace danmaq.Nineball.core.raw {
	...
	public sealed class CLegacyInput : IDisposable {
		...
		public Exception exception {
			...
			private set {
				if( m_exception == null ) { m_exception = value; }
			}
		}
			...
			set {
				if( isCanUseForce ) {
					...
					if( value == EForcePreset.None ) {
						foreach( EffectObject eo in FORCE_PATTERNS.Values ) { eo.Stop(); }
					}
					else {
						EffectObject eo;
						if( FORCE_PATTERNS.TryGetValue( value, out eo ) ) { eo.Start( 1 ); }
					}
				}
			}
		}
		...
		public bool isCanUseForce {
			get { return FORCE_PATTERNS.Count > 0; }
		}
		...
		public void Dispose() {
			try {
				foreach( EffectObject eo in FORCE_PATTERNS.Values ) { eo.Dispose(); }
