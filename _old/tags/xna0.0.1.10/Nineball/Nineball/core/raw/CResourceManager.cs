...
namespace danmaq.Nineball.core.raw {
	...
	public sealed class CResourceManager<_T> where _T : class {
		...
		public void reload( bool bForce, ContentManager mgrContent ) {
			foreach( CResource<_T> resource in resources ) { resource.load( bForce, mgrContent ); }
		}
		...
		public CResource<_T> load( string strAsset, ContentManager mgrContent ) {
			...
			if( mgrContent != null ) { resource.load( true, mgrContent ); }
		...
		/// 
		/// <param name="strAsset">アセット名文字列</param>
		public CResource<_T> load( string strAsset ) { return load( strAsset, null ); }
		...
		public CResource<_T> search( string strAsset, bool bCreate ) {
			...
			if( result == null && bCreate ) { result = load( strAsset ); }
			return result;
		}
		...
		/// <param name="strAsset">アセット名文字列</param>
		/// <returns>アセット名に対応するリソース。存在しない場合、null</returns>
		public CResource<_T> search( string strAsset ) { return search( strAsset, true ); }
