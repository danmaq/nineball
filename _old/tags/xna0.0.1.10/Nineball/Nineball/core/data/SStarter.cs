	...
	public struct SStarter<_T> where _T : new() {
		...
		public struct SFileIOInitializeData {
			...
			private void complete() {
				if( dirContent == null ) { dirContent = ""; }
				if( fileConfigure == null ) { fileConfigure = ""; }
		...
		public struct SXACTInitializeData {
			...
			/// </summary>
			public CallbackIndex2Asset index2assert;
			...
			public override string ToString() {
				...
				if( index2assert == null ) { strResult += Resources.DESC_NULL + Environment.NewLine; }
				else { strResult += index2assert.Method.ToString() + Environment.NewLine; }
				return strResult;
			}
			...
			/// </summary>
			private void complete() {
				if( fileXGS == null ) { fileXGS = ""; }
				if( fileXSB == null ) { fileXSB = ""; }
				if( fileXWBSE == null ) { fileXWBSE = ""; }
				if( fileXWBBGM == null ) { fileXWBBGM = ""; }
			}
		}
		...
		public override string ToString() {
			...
			strResult += "  初回実行シーン         : ";
			if( sceneFirst == null ) { strResult += Resources.DESC_NULL + Environment.NewLine; }
			else { strResult += sceneFirst.ToString() + Environment.NewLine; }
			strResult += "  引数一覧               : ";
			if( args == null || args.Length == 0 ) { strResult += Resources.DESC_NULL + Environment.NewLine; }
			else { strResult += string.Join( " ", args ) + Environment.NewLine; }
		...
		private void complete() {
			...
			if( fps < 1 ) { fps = 60; }
			if( title == null ) { title = strAssemblyName; }
			if( codename == null ) { codename = strAssemblyName; }
			if( args == null ) {
#if WINDOWS
				try { args = Environment.GetCommandLineArgs(); }
				catch( Exception ) { args = new string[ 0 ]; }
#else
				args = new string[ 0 ];
#endif
			}
